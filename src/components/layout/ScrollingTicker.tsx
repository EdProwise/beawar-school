...
import { useScrollWords, useSiteSettings } from "@/hooks/use-school-data";
...
export function ScrollingTicker() {
  ...
  const bgColor = settings?.ticker_bg_color || "#4C0DC9";
  const textColor = settings?.ticker_text_color || "#FFFFFF";

  return (
    <div 
      className="backdrop-blur-md border-y py-2.5 overflow-hidden whitespace-nowrap shadow-lg relative z-40"
      style={{ backgroundColor: bgColor, borderTopColor: `${bgColor}33`, borderBottomColor: `${bgColor}33` }}
    >
        ...
        >
          {scrollingItems.map((item, index) => (
            <div key={`${item.id}-${index}`} className="flex items-center gap-6 group">
              ...
              <span 
                className="text-sm font-black uppercase tracking-[0.25em] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]"
