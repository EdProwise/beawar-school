...
import { useOrbitGroupMessage, useSiteSettings } from "@/hooks/use-school-data";
import { FormattedContent } from "@/components/ui/formatted-content";
import { Loader2, Building2, GraduationCap, Star, Quote, Award, Users } from "lucide-react";
import SEOHead, { buildBreadcrumbSchema } from "@/components/SEOHead";

export function MessageFromOrbitGroup() {
  const { data: message, isLoading } = useOrbitGroupMessage();
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title={`Message from Orbit Group | ${schoolName}`}
        description={`A message from the Orbit Group leadership for ${schoolName}. Committed to transforming education in India.`}
        keywords={`Orbit Group message, ${schoolName} chairman message`}
        canonicalPath="/about/message-from-orbit-group"
        jsonLd={buildBreadcrumbSchema(siteUrl, [{ name: "Home", path: "/" }, { name: "About", path: "/about" }, { name: "Message from Orbit Group", path: "/about/message-from-orbit-group" }])}
      />
      <Header />
      <main>
        ...
        <section className="pt-32 pb-24 bg-gradient-to-b from-primary to-primary-dark relative overflow-hidden">
          ...
          <div className="container relative text-center">
            ...
            <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-extrabold text-primary-foreground mb-5 leading-tight">
              {message?.message_heading || "Message from Orbit Group"}
            </h1>
            ...
            {isLoading ? (
                  ...
                  <div className="sticky top-28 space-y-5">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-2xl">
                        ...
                        <p className="text-sm font-semibold text-foreground leading-tight">Innovate.Educate.Elevate</p>
                      </div>
                    </div>

                      {/* Logos */}
                      <div className="flex flex-col items-center gap-4 pt-2">
                        {/* Orbit Group logo — no circle */}
                        {message.seal_logo_url && (
                          <img
                            src={message.seal_logo_url}
                            alt="Orbit Group Logo"
                            className="w-20 h-20 object-contain"
                          />
                        )}
                        {/* Edprowise logo */}
                        {message.edprowise_logo_url && (
                          <img
                            src={message.edprowise_logo_url}
                            alt="Edprowise Logo"
                            className="w-20 h-20 object-contain"
                          />
                        )}
                    ...
                    <div className="bg-card px-8 md:px-14 py-10 md:py-14 relative overflow-hidden">
                      ...
                      <div className="flex items-end justify-between flex-wrap gap-8">
                        <div>
                          ...
                          <p className="font-heading text-xl font-extrabold text-foreground tracking-tight">
                            {message.sender_name}
                          </p>
                          <p className="text-primary text-sm font-semibold mt-0.5">{message.sender_title}</p>
                          {schoolName && (
                            <p className="text-muted-foreground text-xs mt-1 font-medium">{schoolName}</p>
                          )}
                        </div>
                        ...
                        <div className="shrink-0">
                          <div className="relative w-24 h-24 rounded-full border-[3px] border-primary/30 bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center shadow-inner">
                            ...
                            <div className="text-center z-10">
                              ...
                              <p className="text-[8px] font-black text-primary/70 uppercase tracking-wider mt-0.5 leading-none">
                                Verified
                              </p>
                            </div>
                          </div>
              ...
              <div className="text-center py-32">
                ...
                <p className="text-muted-foreground max-w-sm mx-auto">
                  The leadership message will be published here shortly.
                </p>
              </div>
            )}
          </div>
...
export default MessageFromOrbitGroup;
