...
import { useOrbitManagementTeams, type TeamMember } from "@/hooks/use-school-data";
import { FileUpload } from "@/components/admin/FileUpload";

export default function AdminOrbitManagementTeams() {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const { data: teams, isLoading } = useOrbitManagementTeams();
  ...
  const teamMutation = useMutation({
    mutationFn: async (values: TeamMember[]) => {
      const { data: existingData } = await supabase.from("orbit_management_teams").select("id");
      const existingIds = existingData?.map(v => v.id) || [];
      const currentIds = values.map(v => v.id).filter(id => !id.startsWith("temp-"));
      const toDelete = existingIds.filter(id => !currentIds.includes(id));

      if (toDelete.length > 0) {
        const { error: delError } = await supabase.from("orbit_management_teams").delete().in("id", toDelete);
      ...
      for (const val of values) {
        if (val.id.startsWith("temp-")) {
          const { id, ...rest } = val;
          const { error: insError } = await supabase.from("orbit_management_teams").insert([rest]);
          if (insError) throw insError;
        } else {
          const { error: updError } = await supabase.from("orbit_management_teams").update(val).eq("id", val.id);
          if (updError) throw updError;
        }
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["orbit_management_teams"] });
      toast({ title: "Orbit Management Team saved successfully!" });
    },
    onError: (error) => {
  ...
  const moveMember = (index: number, direction: 'up' | 'down') => {
    ...
    setLocalTeams(newTeams.map((member, idx) => ({ ...member, sort_order: idx })));
  };
    ...
    <AdminLayout>
      <div className="max-w-4xl pb-20">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Orbit Management Team</h1>
            <p className="text-muted-foreground">Manage the Orbit management team members, positions, and descriptions</p>
          </div>
          ...
          <div className="space-y-6">
                  ...
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="e.g., john.doe@orbit.com"
                      />
                    </div>
                    <div>
                      <RichTextEditor
            ...
            {localTeams.length === 0 && (
              <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
                ...
                <p className="text-muted-foreground">No Orbit management team members added yet.</p>
