...
export default function AdminFeesStructure() {
  ...
  const resetForm = () => {
    setForm({ grade: "", admission_fee: "", tuition_fee: "", other_fees: "", total: "" });
    setEditingFee(null);
    setDialogOpen(false);
  };
            ...
            <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate(form); }} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input value={form.tuition_fee} onChange={(e) => setForm({ ...form, tuition_fee: e.target.value })} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Other Fees</Label>
                  <Input value={form.other_fees} onChange={(e) => setForm({ ...form, other_fees: e.target.value })} />
                </div>
                <div>
                  <Label>Total</Label>
            ...
            <tbody className="divide-y divide-border">
              {fees.map((fee: any) => (
                <tr key={fee.id} className="hover:bg-secondary/20 transition-colors">
                  ...
                  <td className="px-6 py-4">{fee.admission_fee}</td>
                  <td className="px-6 py-4">{fee.tuition_fee}</td>
                  <td className="px-6 py-4 font-bold">{fee.total}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="icon" onClick={() => handleEdit(fee)}>
                        <Pencil className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="text-destructive" onClick={() => deleteMutation.mutate(fee.id)}>
